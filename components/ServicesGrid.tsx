...
const ServiceCard: React.FC<ServiceData & { index: number; lang: 'ar' | 'en' }> = ({ icon: Icon, title, subtitle, features, index, lang }) => {
  return (
    <motion.div
      ...
      className="group relative bg-onyx rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 border border-white/5 overflow-hidden flex flex-col h-full"
    >
      ...
      <div className="relative z-10 flex-grow">
        ...
        <h3 className={`text-xl sm:text-2xl md:text-3xl font-bold text-white mb-6 ${lang === 'en' ? 'font-english' : 'font-heading'
          }`}>
          {title}
        </h3>
